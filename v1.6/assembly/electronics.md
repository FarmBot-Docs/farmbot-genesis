...
Once the UTM cable is _physically_ connected to the Farmduino, you need to _electrically_ connect it to the microcontroller using **[jumper wires](../extras/bom/electronics-and-wiring.md#jumper-wire)**. To use the default [UTM pin mapping](../assembly/tools/utm.md#pin-mapping), connect five jumper wires from the upper row of pins to the lower row of pins as highlighted below. You should have one jumper wire connecting `A` to `5V`, `B` to `GND`, `C` to `A9`, `D` to `A5`, and `E` to `D48`.
